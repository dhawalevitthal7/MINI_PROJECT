"""
Pipeline: CouchDB → MySQL
Profiles CouchDB databases, generates MySQL DDL, migrates data.
...
class CouchDBToMySQLPipeline(BasePipeline):
    ...
    def _infer_mysql_type(self, value) -> str:
        ...
        if isinstance(value, str):
            ...
            if length <= 255:
                return "VARCHAR(255)"
            elif length <= _MAX_SAFE_VARCHAR:
                return f"VARCHAR({min(length * 2, _MAX_SAFE_VARCHAR)})"
            else:
                return "LONGTEXT"
    ...
    def execute(
        ...
        for i, mapping in enumerate(mappings):
            ...
            try:
                ...
                # Clean docs (remove CouchDB internals)
                clean_docs = []
                for doc in docs:
                    clean = {}
                ...
                for doc in clean_docs[:100]:
                    for k, v in doc.items():
                        if v is not None and k not in all_columns:
                            all_columns[k] = self._infer_mysql_type(v)
